...
func TestTransferAPI(t *testing.T) {
	...
	// Print account IDs for debugging
	t.Logf("account1 ID: %d", account1.ID)
	t.Logf("account2 ID: %d", account2.ID)
	t.Logf("account3 ID: %d", account3.ID)
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				// Expect GetAccount for from_account_id
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
				// Expect GetAccount for to_account_id
